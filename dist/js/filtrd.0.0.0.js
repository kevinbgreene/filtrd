!function(window, document) {
...
}(window, window.document), function(window, undefined) {
    ...
    var unhandledReasons = [], unhandledRejections = [], trackUnhandledRejections = !0;
    ...
    function Injector() {
        function inject(arr, fn) {
            ...
            return isString(arr) && (arr = [ arr ]), mod = {
                deps: arr,
                fn: fn
            }, isReady ? getDependencies(mod) : (injectorQueue.push(mod), null);
        }
        function _runQueue(arr, fn) {
            if (isReady) throw new Error("Items can only be added to the run queue before run is called");
            runQueue.push({
                deps: arr,
                fn: fn
            });
        ...
        function define(name, arr) {
            if (isUndefined(arr)) return modules[name] || null;
            if (modules[name]) throw new Error("A service with name " + name + " already exists");
            return modules[name] = process(arr), this;
        }
        function process(arr) {
            var fn = null, deps = [];
            return isArray(arr) ? arr.forEach(function(el) {
                isFunction(el) ? fn = el : deps.push(el);
            }) : isDefined(arr) && (fn = arr), {
                fn: fn,
                deps: deps,
                configured: !1
            };
        ...
        function resolve(container, str) {
            ...
            return temp && !temp.configured ? (container[str].fn = getDependencies(temp, str), 
            container[str].fn) : temp && temp.fn ? temp.fn : void 0;
        }
        function getDependencies(mod) {
            var deps = [], fn = mod.fn, i = 0, temp = null;
            for (i = 0; i < mod.deps.length; i++) temp = mod.deps[i], isString(temp) ? deps.push(resolve(modules, temp)) : isFunction(temp) && (fn = temp);
            return fn ? (mod.configured = !0, fn.apply(window, deps)) : null;
        }
        function prepServies() {
            var key = null;
            for (key in modules) resolve(modules, key);
        }
        ...
        function override() {
            prepServies(), isReady = !0, injectorQueue = [], runQueue = [];
        }
        function run() {
            isReady || (prepServies(), clearRunQueue(), clearInjectorQueue(), isReady = !0, 
            injectorQueue = [], runQueue = []);
        }
        var modules = {}, injectorQueue = [], runQueue = [], isReady = !1;
        return {
            define: define,
            inject: inject,
            run: _runQueue,
    ...
    function Injekter() {
        ...
        return injector.define("injekter.config", function() {
            var store = {};
            return {
                get: function(key) {
            ...
            var utility = {};
            return utility.size = size, utility.isUndefined = isUndefined, utility.isDefined = isDefined, 
            utility.isObject = isObject, utility.isString = isString, utility.isNumber = isNumber, 
            utility.isDate = isDate, utility.isArray = isArray, utility.isFunction = isFunction, 
            utility.isBoolean = isBoolean, utility.toDash = toDash, utility;
        }), $(injector.start), {
